...
// 📌 ฟังก์ชันกลางสำหรับอัปโหลดเอกสารเข้า Telegram และรับลิงก์ตรงกลับมา
async function uploadToTelegram(base64Data, fileName, caption) {
  const TELEGRAM_BOT_TOKEN = '8818035747:AAE02Cc1LNx8HjFXosPrMXf9t4KNrBN0T1Q';
  const TELEGRAM_CHAT_ID = '-1004326913622'; 
  ...
  try {
    ...
    const boundary = '-----------------+NodeJSMultipartBoundary' + Math.random().toString(36);
    ...
    postData.push(Buffer.from(`--${boundary}\r\n`));
    postData.push(Buffer.from(`Content-Disposition: form-data; name="${fieldName}"; filename="${fileName}"\r\n`));
    postData.push(Buffer.from(`Content-Type: ${mimeType}\r\n\r\n`));
      ...
      const req = https.request(options, async (res) => {
        ...
        res.on('end', async () => {
          try {
            ...
            if (result.ok) {
              const fileObj = isImage ? result.result.photo[result.result.photo.length - 1] : result.result.document;
              const fileId = fileObj.file_id;

              https.get(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/getFile?file_id=${fileId}`, (fileRes) => {
                let fileData = '';
                ...
                fileRes.on('end', () => {
                  try {
                    ...
                    if (fileJson.ok) {
                      resolve(`https://api.telegram.org/file/bot${TELEGRAM_BOT_TOKEN}/${fileJson.result.file_path}`);
                    } else {
                      resolve(null);
                    }
    ...
    console.error('❌ Telegram Upload Exception:', err.message);
    return null;
  }
}
...
case 'year.save': {
      ...
      case 'year.active': {
        ...
        // 1. เซ็ตปีอื่นๆ ทั้งหมดให้เป็น is_active = false และ status = 'ปิด'
        await supabase.from('AcademicYears').update({ is_active: false, status: 'ปิด' }).neq('id', yearId);

        // 2. เซ็ตปีที่เลือกให้เป็น is_active = true และ status = 'เปิด'
        const { data, error } = await supabase.from('AcademicYears')
          .update({ is_active: true, status: 'เปิด' })
          .eq('id', yearId)
          .select();
...
await writeAudit(currentUser, dataIn.id ? 'behavior.update' : 'behavior.create', 'Behaviors', result.id, { point: result.point });
     ...
     case 'visit.save': {
      ...
      case 'doc.save': {
        ...
        let fileExt = 'jpg'; // ค่าสำรองป้องกัน null
        let fileSize = 0;
        ...
        if (dataIn.file_data && dataIn.file_data.startsWith('data:')) {
          try {
            ...
            if (matches && matches.length === 3) {
              ...
              // 📌 กำหนดข้อความรายละเอียด (Caption) เพื่อให้หน้าแชท Telegram แสดงพรีวิวข้อมูลเอกสารได้อย่างชัดเจน
              const captionText = `📄 เอกสารประจำชั้น\n📌 ชื่อ: ${dataIn.title || 'ไม่มีชื่อ'}\n🏷️ หมวด: ${dataIn.category || '-'}\n📅 วันที่: ${dataIn.doc_date || '-'}\n📝 หมายเหตุ: ${dataIn.note || '-'}`;

              // เรียกอัปโหลดเข้ากลุ่ม Telegram พร้อมแนบ Caption
              uploadedUrl = await uploadToTelegram(dataIn.file_data, fileName, captionText);
            }
          } catch (err) {
        ...
        // 📌 กำหนดค่า URL และประเภทไฟล์ ป้องกันไม่ให้เป็น null เด็ดขาด
        dataIn.file_url = uploadedUrl || dataIn.file_url || 'https://core.telegram.org';
        dataIn.file_type = dataIn.file_type || fileExt;
        dataIn.file_size = dataIn.file_size || fileSize;

        // กรองฟิลด์ Base64 ที่เกินออก
        ['file_data'].forEach(k => delete dataIn[k]);

        // จัดการค่าว่างและตรวจสอบฟิลด์บังคับเพื่อไม่ให้ติด Error ฐานข้อมูล
        Object.keys(dataIn).forEach(key => {
          if (dataIn[key] === '' || dataIn[key] === undefined || dataIn[key] === null) {
            if (key === 'class_id') {
              dataIn[key] = 'ALL';
            } else if (key === 'file_url') {
              dataIn[key] = 'https://core.telegram.org';
...
case 'activity.attend': {
      ...
      case 'home.dashboard': {
        ...
        try {
          const { data: studentsData } = await supabase.from('Students').select('*').eq('status', 'กำลังศึกษา');
        ...
        let todayAttendance = [];
        try {
          const { data } = await supabase.from('Attendance').select('*').eq('date', todayStr);
        ...
        try {
          const { count } = await supabase.from('StudentCases').select('*', { count: 'exact', head: true }).neq('status', 'ปิดเคส');
          openCasesCount = count || 0;
        } catch (e) {}

        let visitsCount = 0;
        try {
          const { count } = await supabase.from('HomeVisits').select('*', { count: 'exact', head: true });
          visitsCount = count || 0;
        } catch (e) {}
        ...
        let tasksList = [];
        try {
          const { data: assigns } = await supabase.from('Assignments').select('*').order('due_date', { ascending: true }).limit(5);
        ...
        // 2. ดึงข้อมูลกิจกรรมล่าสุดพร้อมดึงชื่อนักเรียนและรายละเอียดจริงมาแสดงผล
        let timelineList = [];
        try {
          const { data: logs } = await supabase.from('AuditLogs').select('*').order('at', { ascending: false }).limit(8);
          
          // ดึงข้อมูลตารางที่เกี่ยวข้องมาเตรียมไว้สำหรับเทียบชื่อ
          ...
          (studentsData || []).forEach(s => {
            studentMap[s.id] = `${s.prefix || ''}${s.first_name || ''} ${s.last_name || ''}`.trim();
          });
          ...
          timelineList = (logs || []).map(l => {
            ...
            // แปลงเวลาให้เป็น Timezone ประเทศไทย (Asia/Bangkok)
        ...
        let upcomingList = [];
        try {
          const { data: events } = await supabase.from('CalendarEvents').select('*').gte('date', todayStr).order('date', { ascending: true }).limit(5);
        ...
        // 📌 ปรับมาเรียกใช้ฟังก์ชัน getBehaviorTone(k) ตรงนี้
        ...
        return res.json({
          ...
          kpi: { 
            ...
            male: maleCount,
            female: femaleCount,
            watch: watchStudents.length, 
            rate_today: totalStudents ? Math.round((presentCount / totalStudents) * 100) : 100,
            cases_open: openCasesCount,
            visits: visitsCount,
            contacts_month: contactsMonthCount
          },
          trend: trendData,
